...
- Upstream layer latest/ documents to verify consensus status

## Outputs
- Promoted document in the corresponding latest/ path (status: needs_review)
- Cascade: related docs updated to status: needs_review via update-metadata.sh

## Steps (TODO)
1. Identify source intermediate/ document
2. Validate upstream layer: check that Layer N-1 latest/ has at least one doc with status: consensus
