...
## Purpose
Read the DB90 project state (layer latest/ docs' frontmatter status) and the navigator.csv skill graph to answer "what should I do next?" — recommending the most appropriate skill based on current layer completion and prerequisites.

## Inputs
- .agents/config/_index/navigator.csv (skill graph with phases, prereqs, outputs)
- All layer latest/ documents (reads status frontmatter fields)
...
2. Scan all layers/layer-*/latest/ docs — read status frontmatter for each
